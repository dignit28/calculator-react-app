  ...
  export function negativeExponent(lhs: number, rhs: (number | string)) {
    ...
    if (pow.sign === 'impossible') {
      console.log('Cannot take an odd root from negative number')
      return NaN
...
function rationalizeFloat(targetFloat: number) {
  ...
  let [integerPart, fractionalPart]: [(number|string), (number|string)] = stringifiedFloat.split('.')
  ...
  fractionalPart.match(/(^0*)/)![0].length
  integerPart = +integerPart
  fractionalPart = +`0.${fractionalPart}`
  
  let leftBranchValue: number[] = [0, 1]
  let rightBranchValue: number[] = [1, 0]
  let currentBranchValue: number[] = calculateBranchValue(leftBranchValue, rightBranchValue)
  let currentBranchFloatValue = currentBranchValue[0] / currentBranchValue[1]
  let killswitch = 1000000
  try {
    while (Math.abs(currentBranchFloatValue  - targetFloat) > RAT_FLOAT_EPSILON) {
      if (killswitch-- === 0) {
        throw "Too deep down the Stern-Brocot tree"
      }
      if (currentBranchFloatValue > targetFloat) {
        //Go down to the left branch
        rightBranchValue = currentBranchValue
        currentBranchValue = calculateBranchValue(leftBranchValue, currentBranchValue)
        leftBranchValue = calculateBranchValue(currentBranchValue, rightBranchValue.map(x => -1 * x))
      } else { //if (currentBranchFloatValue < targetFloat)
        //Go down to the right branch
        leftBranchValue = currentBranchValue
        currentBranchValue = calculateBranchValue(rightBranchValue, currentBranchValue)
        rightBranchValue = calculateBranchValue(currentBranchValue, leftBranchValue.map(x => -1 * x))
      }
...
function checkPossibilities(fractionalExponent: string) {
  ...
  let [exponent, root]: [(number|string), (number|string)] = fractionalExponent.split('/')
  exponent = +exponent
  root = +root
  let result: Pow = {
    'exponent': null,
  ...
  function computeSignWhenBothIntegers(localExponent: number, localRoot: number) {
